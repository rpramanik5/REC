...
contract token is ERC721 {
	...
	function balanceOf() public view returns (uint256)  {
     ...
     function ownerOf(uint256 assetId) public view override returns (address) {
         address owner_ = recOwner[assetId];
         require(owner_ != address(0), "NoAssetExists");
         return owner_;
     }
    ...
    function approve(address to,uint256 assetId) public override {
        address owner_ = ownerOf(assetId);
        require(to != owner_, "CurrentOwnerApproval");
        require(msg.sender == owner_,"NotTheAssetOwner");
        assetApprovals[assetId] = to;
        emit Approval(owner_, to, assetId);
    }
    ...
    function isApprovedOrOwner(address spender, uint256 assetId) internal view returns (bool) {
        ...
        address owner_ = ownerOf(assetId);
        return (spender == owner_ || getApproved(assetId) == spender);
    }
